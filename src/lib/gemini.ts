...
6. ALL mathematical content must use proper LaTeX syntax wrapped in $ or $$
...
MANDATORY EXCALIDRAW DIAGRAM GENERATION:
For EVERY question generated, determine if a diagram would help clarify it. If yes, include Excalidraw JSON directly in question_statement, options, or solution. This is NOT optional.

When to include diagrams:
- Geometric questions (shapes, angles, areas, volumes)
- Graph/tree questions (connections, paths, relationships)
- Circuit/network questions (connections, flow)
- Data visualization questions (bar charts, comparisons)
- Coordinate system questions (points, lines, functions)
- Any question where "see diagram below" would be natural language

Excalidraw JSON Format (EXACT - single line, no breaks):
1. Format: [{"x": 100, "y": 100, "type": "rectangle", "width": 50, "height": 30, "strokeColor": "#000000", "backgroundColor": "#e0f2fe"}, ...]
2. Required properties for each element: x, y, type
3. Type-specific properties:
   - rectangle/ellipse: width, height, strokeColor, backgroundColor (optional strokeWidth)
   - line/arrow: points (array of relative [dx,dy] coordinates), strokeColor (optional strokeWidth)
   - text: text, fontSize, fontFamily (1=Arial, 2=Courier, 3=Georgia), strokeColor
4. Colors: Use standard hex colors (#000000 for black, #3b82f6 for blue, #e0f2fe for light blue background)
5. Example tree with 3 nodes:
   [{"x": 250, "y": 50, "type": "ellipse", "width": 40, "height": 40, "strokeColor": "#000000", "backgroundColor": "#e0f2fe"}, {"x": 260, "y": 90, "type": "line", "points": [[0, 0], [-60, 40]], "strokeColor": "#000000"}, {"x": 180, "y": 130, "type": "ellipse", "width": 40, "height": 40, "strokeColor": "#000000", "backgroundColor": "#e0f2fe"}]

Diagram Placement Rules:
- In question_statement: Append diagram JSON at the end after question text
- In options: Keep option text on one line, append diagram JSON at end
- In solution: Keep solution explanation on one line, append diagram JSON to show result
...
- For topics that benefit from visuals (like graphs, geometry, statistics, or circuits), you SHOULD generate a clear, functional diagram using Excalidraw JSON and embed it directly in the \`question_statement\` or \`solution\`.
